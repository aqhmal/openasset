          ...
          <div class="col-md-5">
            ...
            <div class="form-horizontal" role="form">
              <div class="form-group">
                ...
                <div class="col-sm-8">
                  <input type="text" id="name_search" class="form-control" placeholder="Insert user name here..." v-model="name" @change="filterChanged" />
                </div>
              </div>
              <div class="form-group">
                <label for="email_search" class="col-sm-3 control-label">Email:</label>
                <div class="col-sm-8">
                  <input type="email" id="email_search" class="form-control" placeholder="Insert user email here..." v-model="email" @change="filterChanged" />
                </div>
              </div>
            </div>
          </div>
        ...
        <table class="table table-striped table-advance table-hover">
          ...
          <tbody>
            <tr v-for="i in data.data" v-if="data.data.length > 0">
              ...
              <!-- Date Added -->
              <td style="vertical-align: middle;">{{ i.created_at | formattedTime }}</td>
              <!-- Action -->
              <td style="vertical-align: middle;">
                <form v-bind:action="'' + user_url + '/' + i.id + '/delete'" method="post">
                  <input type="hidden" name="_token" v-bind:value="'' + csrf + ''" />
    							<a v-bind:href="'' + user_url + '/' + i.id + '/edit'">
    								<button class="btn btn-primary btn-xs" type="button"><i class="fa fa-edit"></i> Edit</button>
    							</a>
    							<button class="btn btn-danger btn-xs" type="button" @click="deleteUser(i.name)">
